...
- **不排队。** 事件表情期间又来一条事件：新的立即覆盖旧的，计时器重置。同一毫秒内到达多条，取 § 3 优先级表里最高的一条；优先级相同取后到的。

**例外：提交那一下停 600 ms。** `31 接收任务` / `03 好奇` 只是「收到了」的一个点头，
`31` 的切入过渡才 220 ms；停满 1600 ms 会把 `30 思考中` 挡掉大半，看着像卡住。

**例外：回复情绪停 2000 ms。** 1600 ms 是给记忆事件定的——写入、召回都是一闪而过的提示。但情绪推断的结果是**这句回复本身的表情**；按 1600 ms 收掉的话，让丘丘「表演一个生气的表情」，脸上确实变过，只是没人赶得上看见。两者走同一套机制，只是传不同的停留时长（`REPLY_EMOTION_MS`）。取 2000 ms：6000 ms 试过，太黏了，一句话说完脸还僵着好几秒。
...
2. 事件表情未过期（距触发 < 停留时长：记忆事件 1600 ms、回复情绪 2000 ms、提交 600 ms）→ 事件 emotionId
