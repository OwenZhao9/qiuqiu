...
全局快捷键（主进程注册，两个平台各一套）：`Cmd/Ctrl + Shift + E` 唤起桌宠并展开输入条，等价于 `focusPet()` + 展开。**不能用 `Cmd/Ctrl+Shift+Q`**：macOS 上那是系统的「退出登录」，Electron 抢不到时会静默失效，而用户按下去是真的退出登录。
...
| 全局快捷键 | `Cmd/Ctrl + Shift + E` | **无** | 浏览器拿不到全局热键，不提供替代 |
| 托盘图标 | 有 | **无** | — |
