...
| 状态 | 窗口尺寸 | 内容布局 |
|---|---|---|
| 收起 | 200 × 200 | 丘丘 `--qq-ball-size-pet`（200 px）铺满 |
| 展开 | 336 × 256 | 丘丘 200 × 200 居中在上，下方 `--qq-pet-gap`（8 px）间隙，再下是 320 × 40 的输入条，底部留 8 px |

宽度 336 = 320 + 2 × 8 的左右留白。展开与收起时**球心必须不动**：

- 收起 → 展开：窗口宽 +136、高 +56，同时 `window.x -= 68`，`window.y` 不变
- 展开 → 收起：反向
