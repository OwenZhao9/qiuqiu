...
**客户端不再有采集端。** 摄像头那条（每 45 秒抓一帧交给视觉模型）v0.1.19 整条删掉了：
设置页的开关、托盘的暂停项、`ambient.ts`、系统权限申请，全部移除。这个端点保留——
它是记忆层设计好的被动输入口，筛选层的用例也走它——但**应用内没有任何东西会调它**。
声音那一路本来就没实现（缺 VAD 与 ASR，`/health` 的 `missing` 一直报着）。

后果得说清楚：主动输入按 AD-3 跳过筛选，所以删掉摄像头之后，**选择性摄入这一层在真实
使用里没有活的输入源了**，只有演示场景回放会经过它。
...
当前：**v0.1.19**（删掉摄像头；检索规划挪出关键路径）

v0.1.19 两条：**删掉客户端的摄像头采集**（见 § 1 被动采集），**`recall` 的规划器默认改成
确定性的那份**。

规划器这条是量出来的：问模型要一份检索计划实测 2.1 秒，而它落在用户看得见的关键路径上
——检索没出结果，prompt 拼不出来，模型一个字都没开始吐。同一个查询「我下周去哪儿来着」，
模型改写成「用户查询 2026 年 9 月 7 日起一周内的出行计划」只走语义一路；确定性规划改写成
「用户 2026-09-13 去哪儿来着」走语义 + 字面两路——原词保住了，相对日期也解析了，还多一路。
改完首字从 6.2 秒降到 3.8 秒，`meta` 事件从 3.24 秒降到 0.04 秒。要用模型规划设
`RECALL_PLANNER=llm`。

v0.1.18（丘丘不许拿自己说过的话当事实）
