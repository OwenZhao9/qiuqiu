...
| 语音合成走官方接口（豆包语音），不用逆向的 Edge 朗读通道 | 许可证：官方服务条款 |
| 所有记忆数据本地存储，不上传 | 自己定的 |
...
| 语音合成 | `openspeech.bytedance.com`（豆包） | 对话编排 | 无语音，口型不动，文字照常，本次不重试 |
...
- 规则：主窗口订阅自己那只丘丘的 `onEmotion`，每换一次就 `setPetState(state, emotionId)` 发一次；桌宠收到只照做，不推断。表情映射表只在主窗口跑一遍。**闲置推进也算「推断」**：桌宠传 `idle: false`，`idle` 开在主窗口，否则桌宠会照自己的计时器睡过去而主窗口那只还醒着。桌宠窗口中途才起来时，主进程把最后一次 `setPetState` 补发给它——镜像只发变化，不补发的话它停在初始表情上
- 状态：已采纳
...
| 决定 | 为什么能等 |
|---|---|
| 再换别家 TTS（阿里、腾讯） | `TTS` 接口不变，换供应商只加一个适配器文件 |
