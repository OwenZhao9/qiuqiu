...
| edge-tts 只用于 demo，不进产品 | 许可证：非官方接口，无授权条款 |
| 所有记忆数据本地存储，不上传 | 自己定的 |
...
| edge-tts | speech.platform.bing.com | 对话编排 | 无语音，口型不动，文字照常，本次不重试 |
...
| TTS | edge-tts（demo）/ 火山引擎（正式） | 编排 | demo 免费，正式按量 |
...
| 决定 | 为什么能等 |
|---|---|
| 火山 TTS 替换 edge-tts | `TTS` 接口已定，demo 不需正式音色 |
